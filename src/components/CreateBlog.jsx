...
const CreateBlog = () => {
    ...
    return (
        <Box border="1px solid black" padding="50px" borderRadius="12px" display="flex" flexDirection="column" gap="20px" >
            ...
            <TextField style = {{backgroundColor:'#333'}} type="text" placeholder='Enter Blog Title here!' value={blogInfo.title} onChange={(e) => setBlogInfo({ ...blogInfo, title: e.target.value })} />

            <TextField style = {{backgroundColor:'#333'}} type="text" placeholder='Enter Blog Description here!' value={blogInfo.description} onChange={(e) => setBlogInfo({ ...blogInfo, description: e.target.value })} />

            <Box display="flex" gap="4px">
                {
                    categories.map((category) => {
                        return <Chip style = {{backgroundColor:'#333'}} label={category} variant="outlined" onClick={() => handleCategoryClick(category)} />
                    })
                }

            </Box>
            <TextField style = {{backgroundColor:'#333'}} type="text" placeholder='Please Paste url of the image' value={blogInfo.image} onChange={(e) => setBlogInfo({ ...blogInfo, image: e.target.value })} />
